...
function insert_product(code,qty,data,obj) {
	...
	if (data !== undefined) {
		...
		var has_dsc = data.response.member_disc;
		var has_srl = data.response.has_serial;
		var has_flw = data.response.follow_up;
		var product = data.response.product_name;
	} else {
		var has_dsc = has_srl = has_flw = 'N';
		var product = code;
		var code 	= '0000000000000';
		var price 	= qty;
		var qty 	= 1;
	}
	if (obj !== undefined && obj !== false) {
		...
		$(obj).parent().parent().children('.prod_name').html(product);
		$(obj).parent().parent().children().children('.prod_qty').val(qty);
		$(obj).parent().parent().children().children('.prod_price').attr('memberdsc', has_dsc);
		...
		$('#items tr.single_item:last .prod_price').attr('memberdsc', has_dsc);
