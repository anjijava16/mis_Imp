	...
	?>
		...
		<?php
			...
			while ($row = mysql_fetch_assoc($res)) {
				$trstyle = !$trstyle;
				?>
			<tr class="item" style="background-color:<?=$trstyle?'#eee':'#ccc'?>" onClick="document.location='reports-sellday.php?sales_gen&dt1=<?=$_GET['dt1']?>&dt2=<?=$_GET['dt2']?>&cid=<?=$row['c_id']?>&cust=<?=urlencode($row['customer_name'])?>';">
