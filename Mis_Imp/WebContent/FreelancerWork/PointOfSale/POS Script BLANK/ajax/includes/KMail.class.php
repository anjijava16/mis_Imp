<?php
	...
	if (! class_exists('KMail', false))	{
		...
		class KMail	{
			...
			public function addToNames($mails)	{
				if (! isset($mails))	return;
				if (! is_array($mails))	$mails=array($mails);
				$this->to_names=$mails;
			}
			...
			public function report()	{
				if (! $this->report_str)	$this->report_str="\n<br>Mail succesfuly sended without any warning or error.";
			...
			private function mail_create()	{
				...
				if (count($this->to) > 1)	{
					...
					$header		.=	"To: \"{$this->to_names[0]}\" <".$this->to[0].">".$EOL;
				}
				...
				$header 		.=	"Message-ID: <".time()." KMail@".$_SERVER['SERVER_NAME'].">".$EOL
									."X-Mailer: KMail PHP v".phpversion().$EOL
