...
if ($id != "") {
	$update_field = array();
	if (!empty($name)		) $update_field[] = "customer_name='{$name}'";
	if (!empty($tradingas)	) $update_field[] = "customer_tradingas='{$tradingas}'";
	if (!empty($ebayname)	) $update_field[] = "customer_ebay='{$ebayname}'";
	if (!empty($abn)		) $update_field[] = "customer_abn='{$abn}'";
	if (!empty($address)	) $update_field[] = "customer_address='{$address}'";
	if (!empty($shipping)	) $update_field[] = "customer_shipping='{$shipping}'";
	if (!empty($phone)		) $update_field[] = "customer_phone='{$phone}'";
	if (!empty($mobile)		) $update_field[] = "customer_mobile='{$mobile}'";
	if (!empty($email)		) $update_field[] = "customer_email='{$email}'";
	if (!empty($balance)	) $update_field[] = "customer_balance='{$balance}'";
	if (!empty($terms)		) $update_field[] = "customer_terms='{$terms}'";
	if (!empty($discount)	) $update_field[] = "customer_discount='{$discount}'";
	if (!empty($expire)		) $update_field[] = "customer_expire='{$expire}'";
